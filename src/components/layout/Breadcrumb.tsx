  ...
  return (
    <div className="sticky top-16 z-40 bg-primary/90 backdrop-blur-sm">
      <div className="mx-auto flex max-w-5xl items-center gap-2 px-4 py-2 text-sm text-white/70">
        <Link
          to={`/${lang}`}
          className="inline-flex items-center gap-1.5 rounded-md px-1 py-0.5 transition hover:bg-white/5 hover:text-white"
        >
          <House size={16} className="shrink-0" />
          <span>{t('breadcrumb.home')}</span>
        </Link>
        <span className="text-white/30">/</span>
        <span className="inline-flex items-center gap-1.5 px-1 py-0.5 text-white">
          {Icon && <Icon size={16} />}
          {pageName}
        </span>
      </div>
